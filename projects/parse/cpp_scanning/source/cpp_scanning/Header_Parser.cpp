...
namespace cpp_scanning {
  ...
  underworld::Function &Header_Parser::process_common_function_declaration(Context &context) {
    auto name = input.current().get_text();
    auto function = new Function(name, input.get_source_point(), context.get_dungeon());
