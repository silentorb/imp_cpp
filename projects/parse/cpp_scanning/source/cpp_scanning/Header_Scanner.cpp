...
namespace cpp_scanning {
  ...
  void Header_Scanner::load_header(const std::string &path, Lexer_Bundle<runic_cpp::Symbols> lexer_bundle) {
    Common_Lexer<runic_cpp::Symbols> lexer(std::unique_ptr<runic::Text_Source<Char>>(new runic::File_Text_Source<>(path)), lexer_bundle);
    ...
    Header_Parser parser(dungeons, stream, lexer_bundle.lexicon);
    parser.parse();
  }
}